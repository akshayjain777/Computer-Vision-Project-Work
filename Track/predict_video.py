...
parser.add_argument("--court", type=str, default = "AO")
...
if output_video_path == "":
	...
	output_video_path = input_video_path.split('.')[0] + "_TrackNet3_out.mp4"
...
while(True):
    ...
    if circles is not None:
    	...
    	if len(circles) == 1:
            ...
            outvalues.append((currentFrame, x,y))
...
if save_weights_path.split(".")[-1] == '2':
    #pd.DataFrame(outvalues).to_csv("./out/TrackNet2.csv")
    pd.DataFrame(outvalues).to_csv("TrackNet2.csv")
else:
    pd.DataFrame(outvalues).to_csv("TrackNet3.csv")
print ("finish")